...
class Film(models.Model):
    ...
    crew = models.ManyToManyField(Person, through="FilmCrew", related_name="films", blank=True)
    actors = models.ManyToManyField(
        to="films.Actor", through="FilmActor", blank=True, related_name="films", verbose_name="Актеры"
    )
...
class UserFilm(models.Model):
    ...
    user = models.ForeignKey(
        to="users.CustomUser", on_delete=models.CASCADE, related_name="user_films", verbose_name="Пользователь"
    )
    film = models.ForeignKey(Film, on_delete=models.CASCADE, related_name="user_relations")
    ...
    class Meta:
        permissions = [
            ("view_user_films", "Может просматривать фильмы пользователей"),
        ]
